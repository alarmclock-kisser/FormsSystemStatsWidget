...
namespace FormsSystemStatsWidget.Core
{
    public class AudioObj : IDisposable
    {
        ...
        public bool LoadFromFile(string filePath)
        {
            ...
            {
                this.FilePath = string.Empty;
                Logger.Log($"Failed to load audio file: ");
        ...
        public async Task<bool> ResampleAsync(int targetSampleRate, int? targetBitDepth = null)
        {
            ...
            {
                Logger.Log($"Failed to resample audio:");
                Logger.Log(ex.ToString());
                return false;
            }
        }
        ...
        public async Task<bool> RechannelAsync(int targetChannels)
        {
            ...
            try
            {
                return await Task.Run(async () =>
                {
                    ...
                    else
                    {
                        ...
                        Logger.Log($"Unexpected target channel count: {targetChannels}. No rechanneling applied.");
                        return false;
                    }
            ...
            {
                Logger.Log($"Failed to rechannel audio:");
                Logger.Log(ex.ToString());
                return false;
            }
        }
        ...
        public async Task<string?> SerializeAsBase64Async(int? sampleRate = null, int? channels = null, int? bitDepth = null)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                {
                    Logger.Log($"Failed to serialize audio as Base64:");
                    Logger.Log(ex.ToString());
                    return null;
                }
            });
